...
* **Job Order Categories**: here you can add the 'categories'of you projects, choosing a color, a name and various opsions such as:
  ...
  * **Invoice drive budget**: the value of the projects on this category changes based on the total invoiced for that project. This logic also updateds the Invoice Plan and the Production value. You can mark with ‘Invoice Driven Budget’' all the project categories in which you are not sure of the final price because it can vary slightly based on external factors. You can then set a budget with an approximate price and let the issued invoices adjust the final price as a result of the delta between the ‘invoice plan’ and the real invoice.
  * **Project status**
